...
.then(function(products){
    ...
    for(let product of products){
        if (playerQueryString == product.id){
            ...
            out += `
            <img src="${product.img}" alt="Profile" style="width:100%"><h1>${product.name}</h1><p class="title">${product.elo}</p><p>${product.wins} Victoires - ${product.losses} Défaites (${winrate}%)</p><p>Séquence : ${product.streak}</p><br>
            `;
        }
    }
